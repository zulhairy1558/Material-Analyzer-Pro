...
import { calculatePercentage, formatEnergy, formatForce, formatTime } from "@/lib/calculations";
...
export function TearStatsTable() {
  ...
  const rows = useMemo(() => {
    ...
    const out: Array<{
      ...
      timePct: ReturnType<typeof calculatePercentage>;
    ...
    if (ref) {
      ...
      out.push({
        ...
        tearTime: ref.tearTime,
        energy: ref.energy,
        forcePct: calculatePercentage(ref.maxForce, refForce),
        timePct: calculatePercentage(ref.tearTime, refTearTime),
    ...
    datasets.comparisons.forEach((c, i) => {
      ...
      out.push({
        ...
        timePct: calculatePercentage(c.tearTime, refTearTime),
        ...
        <TableBody>
          {rows.map((r, i) => (
            ...
            >
              ...
              <TableCell className="py-2.5 text-right font-mono text-sm tabular-nums">
                {formatTime(r.tearTime, 1)}
              </TableCell>
              <TableCell className="py-2.5 text-right">
                <PercentageBadge result={r.timePct} />
              </TableCell>
              <TableCell className="py-2.5 text-right font-mono text-sm tabular-nums">
                {formatEnergy(r.energy, 1)}
              </TableCell>
              <TableCell className="py-2.5 text-right">
